...
try {
    ...
    // --- STEP 1: Insert bill header (same bills table; reps tagged with created_by) ---
    $createdBy = $conn->real_escape_string((string) currentSection());
    $sql = "INSERT INTO bills (billname, address, phone, total, bill_date,old_payment,discount_type,discount_value, created_by) 
            VALUES ('$bill_name', '$address', '$phone', $total, NOW(), '$old_payment', '$discount_type', $discount_value, '$createdBy')";
    if (!$conn->query($sql)) {
        throw new Exception('Insert bill failed: ' . $conn->error);
    }
    $bill_id = $conn->insert_id;

    // --- STEP 2: Insert bill items + deduct stock + add history ---
    foreach ($data['items'] as $item) {
        $pid   = intval($item['id']);
        ...
        if (isBrandRep()) {
            ...
            if (!$prod || !fusionProductMatchesBrand($prod['name'] ?? '', $prod['category'] ?? '', currentBrandLabel())) {
