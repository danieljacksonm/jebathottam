...
if ($isRep && $brand) {
    ...
    $categories = array_merge(['All'], fusionBillingCategories($allCats));
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Billing by Category</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="assets/fusion_theme.css">
    <style>
        .cat-btn { border-radius: 12px; padding: 10px 20px; margin: 4px; font-weight: 600; min-height: 44px; }
        .cat-btn.active { box-shadow: 0 0 0 3px rgba(15,118,110,0.35); }
        .product-card {
            border-radius: 10px; border: 1px solid var(--fc-border); padding: 12px; margin-bottom: 10px; background: #fff;
        }
        .qty-input, .bill-qty-input, .bill-price-input { font-size: 16px; }
        #productList { max-height: 50vh; overflow-y: auto; -webkit-overflow-scrolling: touch; }
        ...
        .cat-header td { background: #e7f5f2; font-weight: 700; color: #115e59; }
        .mobile-bar {
...
<body class="theme-fusion container-fluid py-3">

    <div class="mb-2"><span class="brand-pill">YEGOVA FUSION CORNER</span></div>
    <div class="card p-3 p-md-4">
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
            <h2 class="mb-0">🧾 Billing by Category<?= $isRep && $brand ? ' — ' . htmlspecialchars($brand) : '' ?></h2>
            <div class="d-flex gap-2 flex-wrap">
                <a href="billing_categories.php" class="btn btn-outline-secondary">🔄 New Bill</a>
                <?php if (!$isRep): ?>
                <a href="billing.php" class="btn btn-outline-primary">📋 Classic Billing</a>
                <?php endif; ?>
        ...
        <div class="row g-2 mb-3">
            <div class="col-12 col-md-3 suggest-wrap">
                <input type="text" id="bill_name" class="form-control form-control-lg" placeholder="Customer / Bill Name" required autocomplete="off">
                <ul id="nameSuggestions" class="list-group suggest-list" style="display: none;"></ul>
            </div>
            <div class="col-12 col-md-3 suggest-wrap">
                <input type="text" id="address" class="form-control form-control-lg" placeholder="Address" autocomplete="off">
                <ul id="addressSuggestions" class="list-group suggest-list" style="display: none;"></ul>
            </div>
            <div class="col-12 col-md-3">
                <input type="text" id="phone" class="form-control form-control-lg" placeholder="Phone" inputmode="tel">
            </div>
        </div>

        <p class="text-muted small mb-2">Choose a category, then add products. Bill items are grouped by category.</p>
        <div class="d-flex flex-wrap gap-2 mb-3">
            <?php foreach ($categories as $cat): ?>
                <button type="button" class="cat-btn btn btn-outline-primary" data-category="<?= htmlspecialchars($cat === 'All' ? '' : $cat) ?>">
        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-6" id="productListWrap">
                ...
                <input type="search" id="productSearch" class="form-control mb-2" placeholder="🔍 Search product..." autocomplete="off">
                ...
                <div id="addCategoryBtnWrap" class="mb-3" style="display: none;">
                    <button type="button" class="btn btn-success w-100 btn-lg" id="addCategoryToBillBtn">Add to Bill</button>
                </div>
            </div>
            ...
            <div class="col-12 col-lg-6">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered bill-table">
                        <thead>
                            <tr>
                                <th>Product</th>
                                <th>Price <small class="opacity-75">(editable)</small></th>
                                <th>Qty <small class="opacity-75">(editable)</small></th>
                                <th>Total</th>
                                <th></th>
                            </tr>
                        </thead>
                ...
                <div class="mt-3 small text-muted">
                    ...
                    <span>Round off: <span id="roundOff">0.00</span></span> &nbsp;|&nbsp;
    ...
    <script>
        ...
        if (urlParams.get('action') === 'edit' && urlParams.get('id')) {
            ...
            fetch('get_bill.php?id=' + editBillId)
                ...
                .then(function(data) {
                    ...
                    billItems = (data.items || []).map(function(item) {
                        return {
                            id: item.id,
                            name: item.name,
        ...
        function loadProducts(category) {
            const url = 'get_products_by_category.php' + (category ? '?category=' + encodeURIComponent(category) : '');
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    list.innerHTML = products.map(p => {
                        ...
                        const cat = p.category || category || 'Other';
                        return `
                            <div class="product-card" data-id="${p.id}" data-name="${escapeHtml(p.name)}" data-price="${p.price}" data-unit="${p.unit||''}" data-mrp="${p.mrp||''}" data-hsn="${p.hsn||''}" data-stock="${stock}" data-freestock="${freeStock}" data-category="${escapeHtml(cat)}">
                                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
                                    <div class="product-name">${escapeHtml(p.name)}</div>
                                    <div class="product-meta">₹${Number(p.price).toFixed(2)} &nbsp;|&nbsp; Stock: ${stock}, Free: ${freeStock}</div>
                                </div>
                                <div class="d-flex align-items-center gap-2 mt-2">
                                    <label class="small mb-0">Qty:</label>
                                    <input type="number" class="form-control form-control-sm qty-input" min="0" value="" placeholder="0" max="${stock}" data-qty inputmode="numeric">
                                    <label class="small mb-0">Free:</label>
                                    <input type="number" class="form-control form-control-sm qty-input" min="0" value="" placeholder="0" max="${freeStock}" data-free inputmode="numeric">
                                </div>
                            </div>
                        `;
                    }).join('');
        ...
        document.getElementById('addCategoryToBillBtn').addEventListener('click', function() {
            ...
            cards.forEach(card => {
                ...
                const category = card.dataset.category || currentCategory || 'Other';
                const total = price * (qty + free);
                billItems.push({ id, name, price, qty, total, mrp, unit, hsn, free, category });
                added++;
                qtyInput.value = '';
                freeInput.value = '';
            });
        ...
        function renderBill() {
            ...
            Object.keys(groups).sort().forEach(cat => {
                tbody.innerHTML += `<tr class="cat-header"><td colspan="5">📂 ${escapeHtml(cat)}</td></tr>`;
            ...
            tfoot.innerHTML = '<tr class="fw-bold"><td colspan="3">Total</td><td>₹' + grandTotal.toFixed(2) + '</td><td></td></tr>';
            ...
            const mob = document.getElementById('mobileGrand');
            if (mob) mob.textContent = rounded.toFixed(2);
        }
        ...
        document.getElementById('checkoutBtn').addEventListener('click', function() {
            ...
            const apiUrl = editBillId ? 'update_bill.php' : 'save_bill.php';
            fetch(apiUrl, {
                method: 'POST',
            ...
            .then(data => {
                const bid = data.trim();
                if (/^\d+$/.test(bid)) {
                    var wasEdit = !!editBillId;
                    window.open('print_bill.php?bill_id=' + bid, '_blank');
                    billItems = [];
                    editBillId = null;
                    renderBill();
                    ...
                    if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'billing_categories.php');
                    alert(wasEdit ? 'Bill updated.' : 'Bill saved. Opening print view.');
                } else {
        ...
        function showSuggestions(field, list) {
            ...
            fetch('get_bill_suggestions.php?q=' + encodeURIComponent(q) + fieldParam)
    ...
    <div class="mobile-bar">
        <div>
            ...
            <div class="fs-5 fw-bold">₹<span id="mobileGrand">0.00</span></div>
        </div>
        <button type="button" class="btn btn-warning btn-lg fw-bold" onclick="document.getElementById('checkoutBtn').click()">Checkout</button>
    </div>
</body>
</html>
