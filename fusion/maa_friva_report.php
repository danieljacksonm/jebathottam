...
function buildInvoiceMap(array $rows)
{
    $map = [];
    $num = 1;
...
$where = "WHERE " . gstReportFilterSql() . " AND " . gstTradersBillFilterSql();
...
$invoiceMap = buildInvoiceMap($rows);
...
if (isset($_GET['download']) && $_GET['download'] === '1') {
    ...
    foreach ($rows as $row) {
        $built = buildSalesRow($row, $invoiceMap[$row['bill_id']]);
        echo '<tr>';
                ...
                <?php else: ?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= intval($row['quantity']) + intval($row['free']) ?></td>
