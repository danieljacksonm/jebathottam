...
$fc = gstConfig();
$companyName  = $fc['company_name'] ?? 'YEGOVA TRADERS';
...
$bankAccount  = $fc['bank_account'] ?? '013100050059415';
$bankIfsc     = $fc['bank_ifsc'] ?? 'TMBL0000013';
...
class PDF extends FPDF
{
    ...
    function BillSectionPart($bill, $x, $y, array $items, $isContinuation = false)
    {
        ...
        if (!$isContinuation) {
            ...
            $this->SetX($x);
            $this->Cell($half, 5, 'Phone: ' . $this->companyPhone, 0, 0, 'L');
            $this->Cell($half, 5, 'Contact: ' . $bill['phone'], 0, 1, 'R');
