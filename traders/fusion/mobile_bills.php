...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == "delete" && !empty($_REQUEST["id"])) {
    ...
    try {
        ...
        $conn->rollback();
        header("Location: mobile_bills.php?error=" . urlencode($e->getMessage()));
        exit;
    }
    header("Location: mobile_bills.php");
    exit;
}
?>
<!DOCTYPE html>
...
<body class="container mt-4">
    ...
    <div class="card p-3 p-md-4">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 gap-2">
            <h2>🧾 Mobile Bills</h2>
            <a href="mobile_hub.php" class="btn btn-success">⬅️ Back</a>
        </div>
<div class="mb-3">
    <button class="btn btn-info" id="showProductsBtn">📦 Show Products & Profit</button>
    <button class="btn btn-warning" id="combineBillsBtn">🧾 Combine Bills</button>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center d-flex flex-wrap justify-content-center gap-1">
                                ...
                                <?php endif; ?>
                                <a href="mobile_bills.php?action=delete&id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete Bill #<?= $row['id'] ?>?')">🗑 Delete</a>
                                <a href="mobile_billing.php?action=edit&id=<?= $row['id'] ?>" class="btn btn-sm btn-primary">✏️ Edit</a>
