...
$fc = fusionConfig();
$companyName = $fc['company_name'] ?? 'YEGOVA FUSION CORNER';
$companyAddr = $fc['company_address'] ?? 'Puthiamputhur, Thoothukudi';
$companyPhone = $fc['company_phone'] ?? '9843059986';
...
$pdf->Cell(95, 6, $companyName, 0, 0, 'L');
...
$pdf->Cell(95, 6, "Contact: " . $companyPhone . " | FSSAI: " . $fssai, 0, 0, 'L');
$pdf->Cell(95, 6, "Address: " . $bill['address'], 0, 1, 'C');
$pdf->Cell(95, 6, "Bill No: " . $bill['id'], 0, 0, 'L');
...
foreach ($dummy_items as $row) {
    ...
    $total  = $row['total'];
    $single = $qty > 0 ? $total / $qty : $price;

    if ($row['unit'] == "Glass Btl") $row['unit'] = "G-Btl";

    $pdf->drawBillRow([
        ['w' => 45, 'text' => $row['name'], 'multiline' => true],
        ['w' => 18, 'text' => $row['hsn']],
        ['w' => 15, 'text' => $row['mrp']],
        ['w' => 12, 'text' => (string) $qty],
        ['w' => 12, 'text' => $row['unit']],
        ['w' => 22, 'text' => number_format($price, 2)],
        ['w' => 22, 'text' => number_format($single, 2)],
        ['w' => 34, 'text' => number_format($total, 2)],
    ]);
...
// --- Totals Row ---
$pdf->SetFont('Arial', 'B', 9);
$pdf->Cell(45, 8, 'Totals', 1);
$pdf->Cell(18, 8, '', 1);
$pdf->Cell(15, 8, '', 1);
$pdf->Cell(12, 8, $total_qty, 1, 0, 'C');
$pdf->Cell(12, 8, '', 1);
$pdf->Cell(22, 8, number_format($total_price, 2), 1, 0, 'C');
$pdf->Cell(22, 8, '', 1);
$pdf->Cell(34, 8, number_format($grand_total, 2), 1, 1, 'C');
$pdf->Ln(10);
