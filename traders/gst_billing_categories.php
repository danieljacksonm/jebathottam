...
<head>
    <meta charset="UTF-8">
    <title>YEGOVA TRADERS — GST Billing</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="assets/traders_theme.css">
    <style>
        .cat-btn { border-radius: 12px; padding: 10px 18px; margin: 4px; font-weight: 600; min-height: 44px; }
        .cat-btn.active { box-shadow: 0 0 0 3px rgba(201,162,39,0.45); }
        .product-card { border-radius: 10px; border: 1px solid var(--yt-border); padding: 12px; margin-bottom: 10px; background: #fff; }
...
<body class="theme-traders container-fluid py-3">
    ...
    <script>
        ...
        if (urlParams.get('action') === 'edit' && urlParams.get('id')) {
            ...
            fetch('get_bill.php?id=' + editBillId)
                ...
                .then(function(data) {
                    ...
                    // Load all bill lines (same as Fusion) — do not filter by GST name
                    billItems = (data.items || []).map(function(item) {
                        var qty = parseInt(item.qty, 10) || 0;
                        var free = parseInt(item.free, 10) || 0;
        ...
        document.getElementById('addCategoryToBillBtn').addEventListener('click', function() {
            ...
            cards.forEach(card => {
                ...
                const total = price * qty; // free is scheme — not charged
        ...
        function renderBill() {
            ...
            billItems.forEach((item, idx) => {
                const paidTotal = (parseFloat(item.price) || 0) * (parseInt(item.qty, 10) || 0);
                item.total = paidTotal;
                const gst = calcGst(paidTotal);
                ...
                tbody.innerHTML += `
                    <tr>
                        ...
                        <td><input type="number" class="form-control form-control-sm bill-qty-input" min="0" value="${item.qty}" data-idx="${idx}" title="Paid qty (free: ${item.free || 0})"></td>
                        <td>${gst.taxable.toFixed(2)}</td>
                        <td>${gst.cgst.toFixed(4)}</td>
                        <td>${gst.sgst.toFixed(4)}</td>
                        <td class="bill-row-total">₹${paidTotal.toFixed(2)}${item.free > 0 ? ' <small class="text-muted">(+'+item.free+' free)</small>' : ''}</td>
            ...
            tbody.querySelectorAll('.bill-price-input').forEach(input => {
                input.addEventListener('change', function() {
                    ...
                    item.price = val;
                    item.total = val * (parseInt(item.qty, 10) || 0);
                    billItems[idx] = item;
                    renderBill();
                });
            });
        ...
        document.getElementById('checkoutBtn').addEventListener('click', function() {
            ...
            // Normalize: GST/total = price * paid qty only; free stays free
            ...
            .then(data => {
                ...
                if (!raw) {
                    alert('Checkout failed: empty response from server. Please re-upload save_gst_bill.php and try again.');
                    return;
                }
                ...
                if (/^[1-9]\d*$/.test(raw)) {
                    ...
                    try {
                        ...
                        if (typeof j === 'number' && j > 0) {
                            bid = String(j);
                        } else if (j && j.bill_id != null && Number(j.bill_id) > 0) {
                            bid = String(j.bill_id);
                        } else {
                            errMsg = 'Server did not return a valid bill id. Response: ' + raw.substring(0, 300);
                        }
                    } catch (e) {
                        errMsg = 'Bad server response: ' + raw.substring(0, 300);
                    }
                }
                ...
                if (/^[1-9]\d*$/.test(bid)) {
                    ...
                    if (window.history && window.history.replaceState) {
                        window.history.replaceState({}, '', 'gst_billing_categories.php');
                    }
                    alert(wasEdit ? 'Bill updated.' : 'Bill saved. Opening print view.');
                    setTimeout(function() { window.location.href = 'gst_bills.php'; }, 600);
                } else {
                    alert(errMsg || ('Checkout failed: ' + raw.substring(0, 300)));
                }
            })
            .catch(function(err) {
                alert('Network error: ' + (err && err.message ? err.message : 'please try again'));
            });
        });
