...
export default function EventDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setEvent(data || null);
      if (data) {
        setRelated(all.filter((e: EventItem) => e.id !== data.id).slice(0, 3));
      }
      setLoading(false);
    }).catch(() => {
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-1">
          {loading ? <p className="text-center text-gray-500">Loading…</p> : <p className="text-center text-gray-500">Event not found.</p>}
          <Link href="/events" className="text-primary-600 hover:underline mt-4 inline-block">← Back to Events</Link>
        </main>
        <Footer />
      </div>
    );
  ...
  const timeStr = event.time ? String(event.time).slice(0, 5) : null;
        ...
        <article className="max-w-5xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  <div className="flex items-center space-x-6">
                    <div className="bg-white/20 backdrop-blur-sm rounded-xl p-6 text-center min-w-[100px] border-2 border-white/30">
                      <div className="text-5xl font-bold mb-1">{new Date(event.date).getDate()}</div>
                    ...
                    <div>
                      ...
                      <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold mb-4 leading-tight">{event.title}</h1>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start">
                  {isUpcoming ? (
                    <div className="w-full bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200 rounded-xl p-6">
                      ...
                      <Button size="lg" className="w-full shadow-md">Register Now</Button>
                    </div>
                  ) : (
          ...
          {event.description && (
            <FadeInUp delay={0.3}>
              <div className="mb-12">
                ...
                <div
                  className="prose prose-lg max-w-none text-gray-700 prose-p:leading-relaxed prose-headings:text-gray-900 prose-a:text-primary-600"
          ...
          {related.length > 0 && (
            <FadeInUp delay={0.5}>
              <div className="mt-16 pt-12 border-t border-gray-200">
                <h3 className="text-3xl font-serif font-semibold text-gray-900 mb-8">Other Events</h3>
                <StaggerContainer>
                          ...
                          <div className="group bg-white rounded-xl shadow-lg hover:shadow-xl transition-all p-6 border border-gray-100 cursor-pointer">
                            <div className="flex items-center space-x-4 mb-4">
                              <div className="bg-primary-600 text-white rounded-lg p-4 text-center min-w-[80px] flex-shrink-0">
                                <div className="text-2xl font-bold">{new Date(relatedEvent.date).getDate()}</div>
                                <div className="text-xs uppercase">{new Date(relatedEvent.date).toLocaleDateString('en-US', { month: 'short' })}</div>
                              </div>
                              <div className="flex-1 min-w-0">
                                <h4 className="font-semibold text-gray-900 group-hover:text-primary-600 transition-colors mb-1 leading-tight">{relatedEvent.title}</h4>
                                <p className="text-sm text-gray-600">{relatedEvent.time ? String(relatedEvent.time).slice(0, 5) : ''}</p>
                              </div>
                            </div>
                            {relatedEvent.description && (
                              <p className="text-gray-700 text-sm line-clamp-2 leading-relaxed mb-3">
                                {relatedEvent.description.replace(/<[^>]+>/g, ' ').trim()}
                              </p>
                            )}
                            <div className="flex items-center justify-between">
                              <span className="px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-xs font-medium">{relatedEvent.type || 'Event'}</span>
          ...
          <FadeInUp delay={0.6}>
            <footer className="mt-16 pt-8 border-t border-gray-200">
              <Link href="/events" className="text-primary-600 hover:text-primary-700 font-medium flex items-center transition-colors">
